...
public class SubMenuPreservationTest {
    ...
    void testMultiMenuFlowPayloadParsingAndPreservation() {
        ...
        // 3. Call generateAiSuggestions on UnifiedAiEngine
        UUID sessionId = UUID.randomUUID();
        UUID tenantId = UUID.randomUUID();
        List<AiSuggestionDto> suggestions = unifiedAiEngine.generateAiSuggestions(
                sessionId, tenantId, flowJson, Collections.emptyList(), "groq", "default", 0.7, 30
        );

        assertNotNull(suggestions);

        // 4. Verify session memory was updated with full 15-node model
        FlowModel activeModel = ServiceRegistry.getFlowContextService().getActiveFlowModel(sessionId);
        assertNotNull(activeModel, "Active flow model must be stored in session memory");
        assertEquals(15, activeModel.getNodes().size(), "Session memory must store full 15 nodes, not stale initial snapshot");
    }
}
