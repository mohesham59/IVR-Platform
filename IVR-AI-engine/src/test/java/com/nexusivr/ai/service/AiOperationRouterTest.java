...
public class AiOperationRouterTest {
    ...
    void testRouteGenerateFlow() {
        ...
        when(unifiedAiEngine.generateFlow(any(), any(), anyString(), any(), any(), anyDouble(), anyInt(), anyList()))
                .thenReturn(mockFlow);

        Object result = router.route(AiOperation.GENERATE_FLOW, sessionId, tenantId, "Build bank", null);

        assertSame(mockFlow, result);
        verify(unifiedAiEngine, times(1)).generateFlow(any(), any(), anyString(), any(), any(), anyDouble(), anyInt(), anyList());
    }
    ...
    void testRouteGenerateFlowWithExplicitProvider() {
        Flow mockFlow = new Flow();
        mockFlow.setName("Gen Flow");
        when(unifiedAiEngine.generateFlow(any(), any(), anyString(), eq("gemini"), eq("gemini-2.0-flash"), anyDouble(), anyInt(), anyList()))
                .thenReturn(mockFlow);
        ...
        assertSame(mockFlow, result);
        verify(unifiedAiEngine, times(1)).generateFlow(any(), any(), anyString(), eq("gemini"), eq("gemini-2.0-flash"), anyDouble(), anyInt(), anyList());
    }
