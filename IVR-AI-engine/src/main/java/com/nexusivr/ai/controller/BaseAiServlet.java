...
public abstract class BaseAiServlet extends HttpServlet {
    ...
    protected String extractModel(HttpServletRequest req) {
        ...
        if (modelHeader != null && !modelHeader.isBlank()) {
            // Validate the model header is actually valid for the requested provider.
            // If a stale model name from a different provider is sent, ignore it and use the provider default.
            if (isModelValidForProvider(provider, modelHeader.trim())) {
                return modelHeader.trim();
            }
            logger.warn("[extractModel] Model '{}' in X-AI-Model header is invalid for provider '{}'. Resetting to provider default.", modelHeader.trim(), provider);
        }
    ...
    /**
     * Returns true if the given model name is valid for the given provider.
     * Returns true for null/blank model (no-op — let the provider decide).
     */
    private boolean isModelValidForProvider(String provider, String model) {
        if (model == null || model.isBlank()) return true;
        return com.nexusivr.ai.ai.ProviderManager.isModelSupportedByProvider(provider, model);
    }
    ...
    protected void handleError(HttpServletResponse resp, Exception e) throws IOException {
        ...
        if (e instanceof ValidationException) {
            ...
            Map<String, String> finalStatuses = providerStatuses.isEmpty() ? providerException.getProviderStatuses() : providerStatuses;
            List<com.nexusivr.ai.dto.common.ProviderAttemptDto> attempts = providerException.getProviderAttempts();

            if (attempts == null || attempts.isEmpty()) {
                String pName = failedProvider != null && !failedProvider.isBlank() ? failedProvider : "provider";
                String attemptReason = e.getMessage() != null && !e.getMessage().isBlank() ? e.getMessage() : failureReason;
                attempts = List.of(new com.nexusivr.ai.dto.common.ProviderAttemptDto(pName, statusCode, attemptReason, retryAfterSeconds));
            }
            if (finalStatuses == null || finalStatuses.isEmpty()) {
                String pName = failedProvider != null && !failedProvider.isBlank() ? failedProvider : "provider";
                finalStatuses = Map.of(pName, failureReason);
            }

            errorResponse.setProviderStatuses(finalStatuses);
            errorResponse.setProviderAttempts(attempts);
