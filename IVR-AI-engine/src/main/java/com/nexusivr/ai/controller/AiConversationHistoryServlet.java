...
public class AiConversationHistoryServlet extends BaseAiServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            if (body != null && body.containsKey("messages") && body.get("sessionId") != null) {
                ...
                String rawBodyTitle = body.get("title") != null ? body.get("title").toString() : null;
                String bodyTitle = com.nexusivr.ai.util.TitleSanitizer.sanitize(rawBodyTitle);
                if (sessionDao.findById(sessionId, tenantId).isEmpty()) {
                    AiSession session = new AiSession();
                    session.setId(sessionId);
                    session.setTenantId(tenantId);
                    session.setChannel(Channel.CHAT);
                    session.setCustomerIdentifier(!bodyTitle.isBlank() ? bodyTitle : "New IVR Flow Session");
                    session.setStatus(SessionStatus.ACTIVE);
                    session.setCreatedAt(java.time.Instant.now());
                    session.setUpdatedAt(java.time.Instant.now());
                    sessionDao.create(session);
                } else if (!bodyTitle.isBlank() && !"New Chat".equalsIgnoreCase(bodyTitle) && !"New IVR Flow Session".equalsIgnoreCase(bodyTitle)) {
                    getChatService().updateSessionTitle(sessionId, tenantId, bodyTitle);
                }
    ...
    protected void doPatch(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            boolean updated = getChatService().updateSessionTitle(sessionId, tenantId, cleanTitle);
            logger.info("Updated title for session {} to '{}': {}", sessionId, cleanTitle, updated);
            sendJsonResponse(resp, HttpServletResponse.SC_OK, Map.of("success", updated, "sessionId", sessionId.toString(), "title", cleanTitle));
        } catch (Exception e) {
