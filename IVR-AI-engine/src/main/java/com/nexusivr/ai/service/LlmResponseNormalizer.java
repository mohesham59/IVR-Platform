...
public class LlmResponseNormalizer {
    ...
    private static String finalCleanup(String text) {
        ...
        // Sanitize cond attribute XML entities (e.g. cond="attempts < 3" -> cond="attempts &lt; 3")
        result = sanitizeCondAttributeXmlEntities(result);

        // Sanitize <else> paired tags (e.g. <else>...</else> -> <else/>...)
        result = sanitizeElseTags(result);
    ...
    public static String sanitizeCondAttributeXmlEntities(String xml) {
        if (xml == null || !xml.contains("cond=")) {
            return xml;
        }
        ...
        while (matcher.find()) {
            String quote = matcher.group(1) != null ? "\"" : "'";
            String val = matcher.group(1) != null ? matcher.group(1) : matcher.group(2);

            // Normalize existing entities first to avoid double-escaping
            String normalized = val.replace("&lt;", "<").replace("&gt;", ">").replace("&amp;", "&");
            // Escape special XML characters inside cond value
            String escaped = normalized.replace("&", "&amp;")
                                       .replace("<", "&lt;")
                                       .replace(">", "&gt;");

            String replacement = "cond=" + quote + escaped + quote;
    ...
    public static String sanitizeElseTags(String xml) {
        if (xml == null || !xml.contains("else")) {
            return xml;
        }

        String result = xml;

        // 1. Convert paired <else>...</else> tags into <else/>...
        Pattern pairedElsePattern = Pattern.compile("<else\\s*>(.*?)</else\\s*>", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
        result = pairedElsePattern.matcher(result).replaceAll("<else/>$1");

        // 2. Convert any remaining unclosed <else> (not <else/>) into <else/>
        Pattern unclosedElsePattern = Pattern.compile("<else(?![\\s/>])\\b|<else\\s*>(?!/)", Pattern.CASE_INSENSITIVE);
        result = unclosedElsePattern.matcher(result).replaceAll("<else/>");

        // 3. Remove any lingering closing </else> tags
        Pattern closingElsePattern = Pattern.compile("</else\\s*>", Pattern.CASE_INSENSITIVE);
        result = closingElsePattern.matcher(result).replaceAll("");
    ...
    private static String escapeXml(String value) {
        ...
        String s = value.replaceAll("&(?!amp;|lt;|gt;|quot;|apos;|#\\d+;|#x[0-9a-fA-F]+;)", "&amp;");
        StringBuilder sb = new StringBuilder();
