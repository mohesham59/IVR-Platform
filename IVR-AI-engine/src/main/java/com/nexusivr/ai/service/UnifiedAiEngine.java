...
public class UnifiedAiEngine {
    ...
    private final com.nexusivr.ai.ai.agents.OptimizationAdvisorAgent optimizationAdvisorAgent;
    private final com.nexusivr.ai.ai.agents.ValidatorAssistantAgent validatorAssistantAgent;
    ...
    public UnifiedAiEngine(ProviderManager providerManager, PromptBuilder promptBuilder,
        ...
        this.optimizationAdvisorAgent = new com.nexusivr.ai.ai.agents.OptimizationAdvisorAgent(providerManager);
        this.validatorAssistantAgent = new com.nexusivr.ai.ai.agents.ValidatorAssistantAgent(providerManager);
    ...
    public Flow generateFlow(UUID tenantId, UUID sessionId, String description,
        ...
        return generateFlow(tenantId, sessionId, description, provider, model, temp, timeout, history, true);
    }

    public Flow generateFlow(UUID tenantId, UUID sessionId, String description,
                             String provider, String model, double temp, int timeout,
                             List<Message> history, boolean autoRefine) {
        logger.info("[UnifiedAiEngine] Starting FLOW GENERATION pipeline. Session: {}. Selected Provider: {}, autoRefine: {}", sessionId, provider, autoRefine);
        ...
        // Pass 1 prompt refinement runs by default unless explicitly disabled (autoRefine=false).
        ...
        logger.info("[UnifiedAiEngine] Pass 1 heuristic status: {}, autoRefine={}", pass1Reason, autoRefine);

        String pass1CacheKey = buildCacheKey(tenantId, description, autoRefine, activeProvider, generationModel, detectedDomain);
        if (!autoRefine) {
            pass1Skipped = true;
            generationInput = description;
            logger.info("[UnifiedAiEngine] Pass 1 skipped per user preference (autoRefine=false). Sending raw prompt to Pass 2.");
        } else {
...
// 3. Pass 2: Generate raw response using the (possibly refined) prompt
         ...
         String pass2CacheKey = buildCacheKey(tenantId, pass2UserPrompt, autoRefine, activeProvider, generationModel, flowDomain);
        ...
        try {
            ...
            String errorMsg = "AI generation failed: LLM output could not be normalized into VoiceXML for domain: " + detectedDomain +
                    ". Normalization error: " + e.getMessage();
            logger.error("[AUDIT] Flow Generation failed at normalization. Throwing ProviderException. domain={}, error={}", detectedDomain, e.getMessage());
        ...
        if (candidateModel == null) {
            ...
            logger.error("[AUDIT] Flow Generation failed after normalization. Throwing ProviderException. domain={}, provider={}, vxmlLength={}, first500={}",
    ...
    /**
     * Generates structured, domain-aware AI Suggestions using specialized LLM agents.
     */
    public List<com.nexusivr.ai.dto.response.AiSuggestionDto> generateAiSuggestions(UUID sessionId, UUID tenantId, String flowJson,
                                                                                      List<ValidationIssueDto> providedIssues,
                                                                                      String provider, String model, double temp, int timeout) {
        String activeProvider = (provider != null) ? provider : GlobalAiConfig.getInstance().getProvider();
        String activeModel = (model != null) ? model : GlobalAiConfig.getInstance().getModel();
        double activeTemp = temp >= 0 ? temp : GlobalAiConfig.getInstance().getTemperature();
        ...
        logger.info("[UnifiedAiEngine] Generating AI Suggestions via LLM Agent (provider={})", activeProvider);

        FlowModel flowModel = resolveExistingModel(flowJson, sessionId);
        if (flowModel == null || flowModel.getNodes().isEmpty()) {
            return java.util.Collections.emptyList();
        }

        List<ValidationIssueDto> issues = providedIssues;
        if (issues == null || issues.isEmpty()) {
            FlowValidationResponse validation = flowModelValidator.validate(flowModel);
            issues = validation.getIssues();
        }

        StringBuilder contextBuilder = new StringBuilder();
        contextBuilder.append("IVR FLOW STRUCTURE:\n");
        contextBuilder.append("Flow Name: ").append(flowModel.getName() != null ? flowModel.getName() : "IVR Flow").append("\n");
        contextBuilder.append("Total Nodes: ").append(flowModel.getNodes().size()).append("\n");
        for (FlowNode n : flowModel.getNodes()) {
            contextBuilder.append("- Node ID: ").append(n.getId())
                    .append(", Type: ").append(n.getType())
                    .append(", Title: '").append(n.getTitle()).append("'");
            if (n.getPrompt() != null && n.getPrompt().getText() != null) {
                contextBuilder.append(", Prompt: '").append(n.getPrompt().getText()).append("'");
            }
            contextBuilder.append("\n");
        }
        contextBuilder.append("\nCONNECTIONS:\n");
        for (FlowConnection c : flowModel.getConnections()) {
            contextBuilder.append("- ").append(c.getSourceNodeId()).append(" (port: ").append(c.getSourcePort())
                    .append(") → ").append(c.getTargetNodeId()).append("\n");
        }

        contextBuilder.append("\nVALIDATION ISSUES & DIAGNOSTICS:\n");
        if (issues != null && !issues.isEmpty()) {
            for (ValidationIssueDto i : issues) {
                contextBuilder.append("- [").append(i.getSeverity()).append("] ")
                        .append(i.getCode()).append(": ").append(i.getMessage())
                        .append(" (nodeId: ").append(i.getNodeId()).append(")\n");
            }
        } else {
            contextBuilder.append("- None detected by rules engine.\n");
        }

        List<com.nexusivr.ai.dto.response.AiSuggestionDto> suggestions = new ArrayList<>();

        // 1. Invoke OptimizationAdvisorAgent & ValidatorAssistantAgent for agent advice
        try {
            String advisorAdvice = optimizationAdvisorAgent.advise(contextBuilder.toString());
            String validatorAdvice = validatorAssistantAgent.advise(contextBuilder.toString());
            logger.info("[UnifiedAiEngine] Specialized agents invoked successfully (Advisor length={}, Validator length={})",
                    advisorAdvice != null ? advisorAdvice.length() : 0, validatorAdvice != null ? validatorAdvice.length() : 0);
        } catch (Exception e) {
            logger.warn("[UnifiedAiEngine] Specialized agent invocation error: {}", e.getMessage());
        }

        // 2. Structured LLM Call to extract actionable JSON suggestions
        String systemPrompt = "You are an expert IVR UX and Routing Consultant. Analyze the provided IVR flow structure and validation issues. " +
                "Generate a JSON array of specific, actionable, human-readable suggestions for the user to review and apply in the IVR Builder UI.\n\n" +
                "CRITICAL RULES:\n" +
                "- Return ONLY a valid JSON array of objects. No markdown formatting, no VoiceXML, no code blocks.\n" +
                "- Each object MUST have fields: \"id\", \"text\", \"icon\", \"severity\", \"nodeId\", \"suggestionType\", \"actionHint\".\n" +
                "- Explicitly reference target nodes by their actual titles and IDs in the text (e.g. \"Add a fallback timeout path from 'Main Menu' (main_menu) to 'End Call'\").\n" +
                "- suggestionType MUST be \"llm_advice\".\n" +
                "- If the flow has broken paths, missing error handlers, or unreachable nodes, create high-priority suggestions addressing them directly by name.\n" +
                "EXAMPLE JSON:\n" +
                "[\n" +
                "  {\"id\": \"sugg_llm_1\", \"text\": \"Add Error/Timeout fallback path for 'Main Menu' (main_menu) to handle caller silence\", \"icon\": \"⚠️\", \"severity\": \"warning\", \"nodeId\": \"main_menu\", \"suggestionType\": \"llm_advice\", \"actionHint\": \"ADD_FALLBACK\"}\n" +
                "]";

        try {
            ProviderManager pm = providerManager != null ? providerManager : new ProviderManager();
            LlmClient client = pm.getLlmClient(activeProvider, activeModel, activeTemp, activeTimeout);
            AiResponse response = client.generateStructuredResponse(systemPrompt, contextBuilder.toString(), null);
            ...
            if (response != null && response.getContent() != null && !response.getContent().isBlank()) {
                ...
                if (content.startsWith("[") && content.endsWith("]")) {
                    ...
                    for (int idx = 0; idx < arr.size(); idx++) {
                        ...
                        String id = obj.has("id") ? obj.get("id").getAsString() : "sugg_llm_" + (idx + 1);
                        String text = obj.has("text") ? obj.get("text").getAsString() : "AI Suggestion for flow optimization";
                        String icon = obj.has("icon") ? obj.get("icon").getAsString() : "💡";
                        String severity = obj.has("severity") ? obj.get("severity").getAsString() : "warning";
                        String nodeId = obj.has("nodeId") ? obj.get("nodeId").getAsString() : null;
                        String actionHint = obj.has("actionHint") ? obj.get("actionHint").getAsString() : "OPTIMIZE";
                        suggestions.add(new com.nexusivr.ai.dto.response.AiSuggestionDto(id, text, icon, severity, nodeId, "llm_advice", actionHint));
                    }
                }
            }
        } catch (Exception e) {
            logger.warn("[UnifiedAiEngine] LLM structured suggestion generation fallback due to: {}", e.getMessage());
        }

        // Grounding Safety Net: if LLM returned empty suggestions but issues exist, generate grounded suggestions referencing node titles by name!
        if (suggestions.isEmpty() && issues != null && !issues.isEmpty()) {
            int idx = 1;
            for (ValidationIssueDto issue : issues) {
                FlowNode n = issue.getNodeId() != null ? flowModel.getNode(issue.getNodeId()) : null;
                String nodeTitle = n != null && n.getTitle() != null ? n.getTitle() : (issue.getNodeId() != null ? issue.getNodeId() : "Flow");
                String text;
                String icon = "💡";
                String severity = issue.getSeverity() == ValidationSeverity.ERROR ? "error" : "warning";

                if (issue.getMessage().contains("unreachable") || issue.getMessage().contains("disconnected")) {
                    text = "AI Suggestion: Reconnect unreachable node '" + nodeTitle + "' to complete caller routing";
                    icon = "🔗";
                } else if (issue.getCode().equals("DELETED_BRANCHING_HUB") || issue.getMessage().contains("converge")) {
                    if (issue.getNodeId() != null && flowModel != null) {
                        boolean alreadyConnected = flowModel.getConnections().stream().anyMatch(c -> {
                            FlowNode src = flowModel.getNode(c.getSourceNodeId());
                            FlowNode tgt = flowModel.getNode(c.getTargetNodeId());
                            boolean srcIsMenu = src != null && src.getType() == FlowNodeType.MENU;
                            boolean tgtIsMenu = tgt != null && tgt.getType() == FlowNodeType.MENU;
                            return (srcIsMenu && issue.getNodeId().equals(c.getTargetNodeId())) ||
                                   (tgtIsMenu && issue.getNodeId().equals(c.getSourceNodeId()));
                        });
                        if (alreadyConnected) {
                            logger.info("[UnifiedAiEngine] Skipping redundant menu suggestion for node '{}' — already connected to Menu", issue.getNodeId());
                            continue;
                        }
                    }
                    text = "AI Suggestion: Re-insert Menu node for '" + nodeTitle + "' to fix unbranched routing";
                    icon = "🔀";
                } else if (issue.getMessage().contains("No End node")) {
                    text = "AI Suggestion: Connect terminating path from '" + nodeTitle + "' to End Call node";
                    icon = "🔚";
                } else {
                    text = "AI Suggestion: Address " + issue.getCode() + " on '" + nodeTitle + "' (" + issue.getMessage() + ")";
                }

                suggestions.add(new com.nexusivr.ai.dto.response.AiSuggestionDto("sugg_llm_grounded_" + idx, text, icon, severity, issue.getNodeId(), "llm_advice", issue.getCode()));
                idx++;
            }
        }

        // Deduplicate suggestions by (nodeId + text)
        Set<String> seenKeys = new HashSet<>();
        List<com.nexusivr.ai.dto.response.AiSuggestionDto> deduplicated = new ArrayList<>();
        for (com.nexusivr.ai.dto.response.AiSuggestionDto sugg : suggestions) {
            String normText = sugg.getText() != null ? sugg.getText().toLowerCase().trim().replaceAll("\\s+", " ") : "";
            String normNodeId = sugg.getNodeId() != null ? sugg.getNodeId().toLowerCase().trim() : "global";
            String key = normNodeId + "::" + normText;
            if (seenKeys.add(key)) {
                deduplicated.add(sugg);
            }
        }

        logger.info("[UnifiedAiEngine] Produced {} LLM-driven suggestion(s) for flow (deduplicated from {})", deduplicated.size(), suggestions.size());
        return deduplicated;
    }

    /**
     * Applies a specific AI suggestion (or all pending suggestions) using targeted LLM patch operations.
     */
    public FlowImprovementResponse applySuggestion(UUID sessionId, String existingFlowJson, String suggestionText,
                                                  String provider, String model, double temp, int timeout) {
        logger.info("[UnifiedAiEngine] Starting AGENT-DRIVEN SUGGESTION APPLICATION pipeline. Session: {}, Suggestion: '{}'", sessionId, suggestionText);
        String instruction = "Apply the following specific AI suggestion to optimize the IVR flow structure: " + suggestionText;
        return improveFlowWithPatches(sessionId, existingFlowJson, instruction, provider, model, temp, timeout);
    }
    ...
    private static String buildCacheKey(UUID tenantId, String promptOrSpec, boolean autoRefine, String provider, String model, String domain) {
    ...
    private static String buildCacheKey(String promptOrSpec, boolean autoRefine, String provider, String model, String domain) {
        return buildCacheKey(null, promptOrSpec, autoRefine, provider, model, domain);
    }
}
