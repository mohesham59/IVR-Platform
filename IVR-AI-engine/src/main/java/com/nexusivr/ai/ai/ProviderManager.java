...
public class ProviderManager {
    ...
    /**
     * Returns true if the given model is valid for the given provider.
     * Case-insensitive model name matching.
     */
    public static boolean isModelSupportedByProvider(String provider, String model) {
        if (provider == null || provider.isBlank() || model == null || model.isBlank()) return true;
        List<String> models = PROVIDER_MODELS.get(provider.toLowerCase());
        if (models == null || models.isEmpty()) return true; // Unknown provider — let the backend decide
    ...
    public LlmClient getLlmClient(String providerName, String modelName, double temperature, int timeoutSeconds) {
        ...
        if ("ollama".equalsIgnoreCase(provider)) {
            ...
            // Use Math.max so callers requesting e.g. 20s don't clamp OpenRouter below its configured 35s minimum.
            timeoutSeconds = Math.max(timeoutSeconds, com.nexusivr.ai.config.LlmConfig.getOpenRouterTimeout());
        }
    ...
    private CircuitBreaker getCircuitBreaker(String provider) {
        return circuitBreakerMap.computeIfAbsent(provider, p -> new CircuitBreaker(p));
    }
  ...
  *
   ...
   * @throws ProviderException when every provider is unavailable after retries
    ...
    public AiResponse executeWithRetryAndFallback(String provider, String model, double temp,
        ...
        for (String targetProvider : providersToTry) {
            if (!isProviderAvailable(targetProvider)) {
                ...
                if (breaker.getState() == CircuitBreaker.State.OPEN) {
                    if (breaker.isAuthFailure() || health.isAuthFailure()) {
                        statusCode = 401;
                        skipReason = "Authentication failed — check API key configuration";
                    } else {
                        statusCode = 429;
                        skipReason = "Quota exceeded, retry after ~" + Math.max(1, (cooldownSecs + 59) / 60) + " min";
                    }
                } else if (health.isUnavailable()) {
                    statusCode = health.isAuthFailure() ? 401 : (health.isRateLimited() ? 429 : 503);
            ...
            if (statusCode == 401 || statusCode == 403) {
                attemptReason = "Authentication failed — check API key configuration";
        ...
        throw new ProviderException(originalRequestedProvider, errorMsg, reason, providerStatuses, providerAttempts);
    }
    ...
    private AiResponse attemptWithRetries(LlmClient client, String provider, String model, double temp,
        ...
        for (int attempt = 1; attempt <= MAX_RETRIES_PER_PROVIDER; attempt++) {
            ...
            AiResponse response;
            long taskTimeoutMs = Math.max(10, timeout > 0 ? timeout : 30) * 1000L + 5000L;
            try {
                if (structuredOutput) {
                    response = com.nexusivr.ai.ai.executor.AiTaskExecutor.execute(
                            () -> client.generateStructuredResponse(systemInstruction, userPrompt, List.of()),
                            taskTimeoutMs
                    );
                } else {
                    response = com.nexusivr.ai.ai.executor.AiTaskExecutor.execute(
                            () -> client.generateResponse(systemInstruction, userPrompt, List.of()),
                            taskTimeoutMs
                    );
                }
            } catch (Exception e) {
